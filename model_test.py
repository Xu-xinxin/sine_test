...
def model(n_mid, embedding_dim, hidden_size, batch_size, seq_len, topic_num, category_num, alpha,
    ...
    n_size = n_mid  # item总数
    lr = 0.001
    hist_max = seq_len  # 用户行为序列长度
    dim = embedding_dim
    share_emb = share_emb
    with tf.name_scope('Inputs'):
        # i_ids: label
        i_ids = tf.placeholder(shape=[None], dtype=tf.int32)
        # item: hist_item_list
        item = tf.placeholder(shape=[None, seq_len], dtype=tf.int32)
        # nbr_mask: hist_mask_list
        nbr_mask = tf.placeholder(shape=[None, seq_len], dtype=tf.float32)
